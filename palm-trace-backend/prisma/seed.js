...
const main = async () => {
  try {
    ...
    await prismaClient.akun.upsert({
      where: { email: 'unilever.admin@palmtrace.co.id' },
      update: {},
      create: {
        email: 'unilever.admin@palmtrace.co.id',
        password: await argon2.hash('Unilever@PalmTrace2023'),
        role: util.getAttributeName('unilever').databaseRoleName,
        [util.getAttributeName('unilever').tableName]: {
          create: {
            nama: 'Unilever Admin',
            alamat: 'Bandung',
